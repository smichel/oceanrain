...
if dataread == 0
    ...
    if meteor == 1
        data=load('/data/mpi/eo/workspace/u231036/ODM470/ODM470_linux/meteor/final_data/joint_METEORDISDRO_M105-M124_colloc_cont_ww_nc.txt');
    end
    
    % Investigator data:
    
    if investigator == 1
        data=load('/scratch/uni/u237/users/smichel/oceanrain/investigator/joint_investigator_disdro_2016T01-2016V04_colloc_cont_ww_nc.txt');
    end
    
%      Polarstern data:
    
    if polarstern == 1
        data=load('/data/mpi/eo/workspace/u300499/polarstern/data/joint_data/joint_PSDISDRO_PS76-PS97_colloc_cont_ww_nc_newLatLon.txt');
    end
    
%      Sonne data:
    
    if sonne == 1
        data=load('/data/mpi/eo/workspace/u300499/sonne2/data/joint_data/joint_SONNEDISDRO_SO237-SO246_colloc_cont_ww_nc_newLatLon.txt');
    end
    
%       Merian data:

    if merian == 1
        data=load('/data/mpi/eo/workspace/u231036/ODM470/ODM470_linux/merian/or_postprocessing/data/joint_data/joint_merian_disdro_MSM21-MSM38_colloc_cont_ww_nc.txt');
    end
    ...
    for i=1:length(lon)
        ...
        if mod(i,timestep) == 0
            ...
            if ~isnan(h_w_10m(:,k)) & ~isnan(h_Twater(:,k)) & ~isnan(h_T(:,k)) & ~isnan(h_q(:,k)) & ~isnan(h_P_air(:,k)) & ~isnan(h_R_s(:,k)) & ~isnan(h_R_l(:,k)) & ~isnan(h_rain(:,k))
                [cd, ce, ch, h_webb, h_rain, h_sst, h_shf ,h_lhf,h_evap]=coare3_0(h_date(:,k),h_w_10m(:,k),h_Twater(:,k),h_T(:,k),h_q(:,k),h_P_air(:,k),h_R_s(:,k),h_R_l(:,k),h_rain(:,k),h_lat(:,k),h_lon(:,k),win_height,hum_height,temp_height,bulktemp_depth,warmlayer);
