...
data_name='joint_merian_disdro_MSM21-MSM38_colloc_cont_ww_na.txt';
data=load('joint_merian_disdro_MSM21-MSM38_colloc_cont_ww_na.txt');
tocheck=load('joint_merian_disdro_MSM21-MSM38_phase_algo_prelim_tocheck.txt');

PSD_na_name='joint_merian_disdro_MSM21-MSM38_psd_na.txt';
PSD_num_na_name='joint_merian_disdro_MSM21-MSM38_psd_num_na.txt';
PSD_na=load(PSD_na_name);
PSD_num_na=load(PSD_num_na_name);
rain_snow=load('rainsnow_data_MSM21-MSM38.txt');
rain=load('rain_minute_psd_num_MSM21-MSM38.dat');
...
rain_PSD_na=load('rain_minute_psd_dbz_MSM21-MSM38.dat');
rain_PSD_num_na=load('rain_minute_psd_num_MSM21-MSM38.dat');


data(310613,17)=data(310612,17)/2+data(310614,17)/2;
changed={[data(310613,6) 17]};
k=1;
...
[check_lia,check_loc]=ismember(resc_rain,tocheck(:,3));
...
for i=1:length(resc_rain)
    ...
    %PSD_num
        PSD_pos=find(resc_rain(i)<PSD_na(:,6),1,'first');
        PSD_na_dummy=zeros(1,149);
        PSD_num_na_dummy=PSD_na_dummy;
        PSD_na_dummy(1)=PSD_pos;
        PSD_na_dummy(2:4)=rain_snow(rain_loc_fillin_na(i),2:4);
        PSD_na_dummy(5)=data(loc(i),5);
        PSD_na_dummy(6:8)=tocheck(check_loc(i),3:5);
        PSD_na_dummy(9)=1; % Rain probability set on 1
        PSD_na_dummy(10:11)=0; % Snow and mixphase probability set on 0
        ...
        PSD_na_dummy(14)=rain_snow(rain_loc_fillin_na(i),6); % num_bins
        PSD_na_dummy(15)=rain_snow(rain_loc_fillin_na(i),7); % num_particles
        PSD_na_dummy(16)=rain_snow(rain_loc_fillin_na(i),5); % preciprate
        PSD_na_dummy(17)=rain_snow(rain_loc_fillin_na(i),8); % reflectivity
        PSD_na_dummy(18)=rain_snow(rain_loc_fillin_na(i),9); % dBr
        PSD_na_dummy(19)=rain_snow(rain_loc_fillin_na(i),10); % dBZ
        PSD_na_dummy(20:21)=rain_snow(rain_loc_fillin_na(i),17:18);
        PSD_na_dummy(22:149)=rain_PSD_na(rain_loc_fillin_na(i),5:132); %bins
               
        %PSD_num_na
        
        %flag 1: switching it to mixed phase
        PSD_num_na_dummy(1)=PSD_pos;
        PSD_num_na_dummy(2:4)=rain_snow(rain_loc_fillin_na(i),2:4);
        PSD_num_na_dummy(5)=data(loc(i),5);
        PSD_num_na_dummy(6:8)=tocheck(check_loc(i),3:5);
        PSD_num_na_dummy(9)=1; % Rain probability set on 1
        PSD_num_na_dummy(10:11)=0; % Snow and mixphase probability set on 0
        
        PSD_num_na_dummy(12)=0;
        %flag 2: switching it to the according precip intensity
        if rain_snow(rain_loc_fillin_na(i),5) ==0
            PSD_num_na_dummy(13) = 12;
        elseif rain_snow(rain_loc_fillin_na(i),5)<=0.09
            PSD_num_na_dummy(13) = 13;
        elseif rain_snow(rain_loc_fillin_na(i),5)>0.09 & rain_snow(rain_loc_fillin_na(i),5)<=0.99
            PSD_num_na_dummy(13) = 14;
        elseif rain_snow(rain_loc_fillin_na(i),5)>0.99 & rain_snow(rain_loc_fillin_na(i),5)<=10
            PSD_num_na_dummy(13) = 15;
        elseif rain_snow(rain_loc_fillin_na(i),5)>10   & rain_snow(rain_loc_fillin_na(i),5)<=50
            PSD_num_na_dummy(13) = 16;
        elseif rain_snow(rain_loc_fillin_na(i),5)>50
            PSD_num_na_dummy(13) = 17;
        end
        
        PSD_num_na_dummy(14)=rain_snow(rain_loc_fillin_na(i),6); % num_bins
        PSD_num_na_dummy(15)=rain_snow(rain_loc_fillin_na(i),7); % num_particles
        PSD_num_na_dummy(16)=rain_snow(rain_loc_fillin_na(i),5); % preciprate
        PSD_num_na_dummy(17)=rain_snow(rain_loc_fillin_na(i),8); % reflectivity
        PSD_num_na_dummy(18)=rain_snow(rain_loc_fillin_na(i),8); % dBr
        PSD_num_na_dummy(19)=rain_snow(rain_loc_fillin_na(i),9); % dBZ
        PSD_num_na_dummy(20:21)=rain_snow(rain_loc_fillin_na(i),17:18);
        PSD_num_na_dummy(22:149)=rain_PSD_num_na(rain_loc_fillin_na(i),5:132); %bins
    
        PSD_na=vertcat(PSD_na(1:PSD_pos-1,:),PSD_na_dummy,PSD_na(PSD_pos:end,:));
        PSD_num_na=vertcat(PSD_num_na(1:PSD_pos-1,:),PSD_num_na_dummy,PSD_num_na(PSD_pos:end,:));
end
toc
doubles_headers=find(diff(PSD_num_na(:,6))==0)+1;
PSD_na(doubles_headers,:)=[];
PSD_num_na(doubles_headers,:)=[];
...
if safe
    ...
    parameter_outfile='joint_merain_disdro_MSM21-MSM38_changed_parameters.txt';
