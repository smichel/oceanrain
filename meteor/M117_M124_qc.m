...
data=load('joint_METEORDISDRO_M117-M124_colloc_cont_ww_na.txt');
data_name='joint_METEORDISDRO_M117-M124_colloc_cont_ww_na.txt';

data_name='joint_METEORDISDRO_M117-M124_colloc_cont_ww_na.txt';
PSD_na_name='joint_METEORDISDRO_M117-M124_psd_na.txt';
PSD_num_na_name='joint_METEORDISDRO_M117-M124_psd_num_na.txt';
PSD_na=load(PSD_na_name);
PSD_num_na=load(PSD_num_na_name);
rain_snow=load('rainsnow_data_M117-M124.txt');
rain=load('rain_minute_psd_num_M117-M124.dat');

snow_PSD_num_na=load('snow_minute_psd_num_M117-M124.dat');
snow_PSD_na=load('snow_minute_psd_dbz_M117-M124.dat');

rain_PSD_num_na=load('rain_minute_psd_num_M117-M124.dat');
rain_PSD_na=load('rain_minute_psd_dbz_M117-M124.dat');
...
% plot(data(:,8),data(:,7),'.','MarkerSize',0.1,'Color',[0 0 0])
%     x = data(:,8);
%     y = data(:,7);
%     C = zeros(length(data(:,23)),1);
%     for i=1:length(data(:,23))
%        C(i) = data(i,23);
%     end
%     Sal_max=max(data(:,23));    
%     hold on
%     cdivs = size(cmap,1);
%     edges=linspace(0,Sal_max,cdivs+1);
...
%     hcb=colorbar;
%     set(hcb,'Ytick',0:5:65,'Yticklabel',0:Sal_max/14:Sal_max)
%     hcb.Label.String = 'Salinity in PSU';
...
k=1;
for i=1:size(data,1)
    if data(i,23) < 5 & data(i,23) ~= -99.99 
        data(i,23)=-99.99;
