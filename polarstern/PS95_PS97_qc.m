...
data=load('joint_PSDISDRO_PS95-PS97_colloc_cont_ww_na.txt');

tocheck=load('joint_PSDISDRO_PS95-PS97_phase_algo_prelim_tocheck.txt');

data_name='joint_PSDISDRO_PS95-PS97_colloc_cont_ww_na.txt';
PSD_na_name='joint_PSDISDRO_PS95-PS97_psd_na.txt';
PSD_num_na_name='joint_PSDISDRO_PS95-PS97_psd_num_na.txt';
PSD_na=load(PSD_na_name);
PSD_num_na=load(PSD_num_na_name);
rain_snow=load('rainsnow_data_PS95-PS97.txt');
rain=load('rain_minute_psd_num_PS95-PS97.dat');

snow_PSD_num_na=load('snow_minute_psd_num_PS95-PS97.dat');
snow_PSD_na=load('snow_minute_psd_dbz_PS95-PS97.dat');

rain_PSD_num_na=load('rain_minute_psd_num_PS95-PS97.dat');
rain_PSD_na=load('rain_minute_psd_dbz_PS95-PS97.dat');
...
data(130594,11)=data(130594-1,11)/2+data(130594+1,11)/2;
changed={[data(130594,6) 11]};

k=1;
for i=2:size(data,1)
    if data(i,10) == 0 & data(i-1,10) ~= 0 & data(i+1,10)~= 0 & i < 70000
        susp(k)=i;
        k=k+1;
    end
end
...
data(133411,11)=data(133411-1,11)/2+data(133411+1,11)/2;
changed=[changed,{[data(133411,6) 11]}];
...
[snow_lia_na,snow_loc_na]=ismember(data(snow_susp,6),PSD_na(:,6));
